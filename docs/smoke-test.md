...
**Recommended: run `bash scripts/smoke.sh` instead** of typing the call by hand — the script writes the `Authorization: Bearer …` header into a `chmod 600` `curl -K` config file so the session id is never visible in `ps`, `.bash_history`, or shell `set -x` output. Use the manual form below only when you specifically want to step through the call.

If you do want the manual version, mirror the script's pattern so the bearer token is not put on the command line (where any local process can read it via `/proc/<pid>/cmdline`):

```bash
source .env
TMP="$(mktemp -d)"; trap 'rm -rf "$TMP"' EXIT
umask 077
# Authorization header lives in a mode-600 config file, NOT in argv.
printf 'header = "Authorization: Bearer %s"\n' "${AGENTICPLUG_SESSION}" > "$TMP/curl.cfg"
printf 'header = "Content-Type: application/json"\n' >> "$TMP/curl.cfg"
printf '%s' "{\"model\":\"${OLLAMA_MODEL}\",\"messages\":[{\"role\":\"user\",\"content\":\"Reply with exactly the single word: pong.\"}],\"temperature\":0,\"stream\":false}" > "$TMP/body.json"
chmod 600 "$TMP/curl.cfg" "$TMP/body.json"
curl -sS -K "$TMP/curl.cfg" --max-time 120 \
...
Avoid the convenient-looking `curl -H "Authorization: Bearer $TOKEN" …` form: the header value ends up in `argv` and any user on the box can read it via `ps -ef` or `/proc`. If you must use it on a single-user development laptop, at least be aware of the trade-off and prefer `scripts/smoke.sh` for everything else.
