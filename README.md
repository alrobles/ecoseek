...
**Frontend ↔ Emily proxy auth:**

| Variable | Default | Description |
|---|---|---|
| `EMILY_API_KEY` | _(generated by `setup.sh`)_ | Shared secret. The Emily backend validates it as `API_SERVER_KEY`; the frontend uses it as the default for `EMILY_HERMES_KEY`. |
| `EMILY_HERMES_KEY` | `${EMILY_API_KEY}` | Bearer token the frontend's nginx injects server-side when proxying `/v1/` → Emily on `:8642`. **Must be non-empty** (matching the backend key) or the frontend container refuses to start — an empty value silently produces `HTTP 401` on every chat. Override only when the `:8642` backend uses a different key (e.g. a host hermes process). |
