...
class QueryEndpointTests(unittest.TestCase):
    ...
    def test_messages_win_when_text_also_present(self):
        with patch.object(api, "_try_agenticplug_chat", new=AsyncMock(return_value="agentic response")) as mocked:
        ...
        mocked.assert_awaited_once_with("use this", [{"role": "user", "content": "use this"}])
    ...
    def test_auto_falls_back_to_local(self):
        with (
            ...
            patch.object(api, "_try_agenticplug_chat", new=AsyncMock(return_value=None)),
            patch.object(api, "_try_local_llm", new=AsyncMock(return_value="local response")),
        ):
            response = self.client.post("/v1/query", json={"mode": "auto", "text": "hello"})
    ...
    def test_explicit_local_mode_skips_other_upstreams(self):
        with (
            ...
            patch.object(api, "_try_agenticplug_chat", new=AsyncMock(return_value="wrong")),
            patch.object(api, "_try_local_llm", new=AsyncMock(return_value="local response")) as mocked_local,
        ):
            response = self.client.post("/v1/query", json={"mode": "local", "text": "hello"})
...
class LocalEndpointTests(unittest.IsolatedAsyncioTestCase):
    async def test_local_llm_uses_openai_path(self):
        ...
        with (
            ...
            answer = await api._try_local_llm("hello", [{"role": "user", "content": "hello"}])
