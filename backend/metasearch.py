...
def _geo_boost(papers, geo_context):
    ...
    logger.info(
        "Geo filter: %d/%d papers mention '%s'", len(boosted), len(papers), geo_context
    )
    return boosted + rest
...
def metasearch(user_query):
    ...
    if cached:
        ...
        logger.info(
            "Translated: '%s' → '%s' (geo=%s)", user_query[:40], english[:40], geo
        )
    ...
    if len(papers) < 5 and geo:
        logger.info(
            "Too few geo-filtered results (%d), retrying without filter", len(papers)
        )
